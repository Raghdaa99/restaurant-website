<template>
  <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center p-3 sm:p-4 border-b border-slate-300 last:border-b-0 hover:bg-slate-300/50 transition-all duration-300">
    <div class="col-span-12 md:col-span-6 flex items-center gap-3 sm:gap-4">
      ...
      >
      <div>
        <h3 class="font-semibold text-base sm:text-lg text-slate-800">{{ item.name }}</h3>
        <p class="text-sm sm:text-base text-slate-600">${{ formatPrice(item.price) }}</p>
      </div>
    </div>
    ...
    <!-- Quantity Column -->
    <div class="col-span-12 md:col-span-3 flex items-center md:justify-center gap-2">
      <span class="text-sm md:hidden">Quantity:</span>
      <div class="flex items-center bg-slate-100 rounded-lg border border-slate-300 shadow-sm">
        <button 
        ...
        <span class="w-8 sm:w-12 text-center py-1 sm:py-2 text-slate-700">{{ item.quantity }}</span>
        <button 
